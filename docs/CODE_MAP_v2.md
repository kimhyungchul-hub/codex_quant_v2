...
| 날짜 | 내용 | 파일 |
|------|------|------|
| 2026-02-16 | XRP 수동거래 제외 기반 재평가/운영 우선순위 반영: CF·원인매트릭스·배치 손실드라이버·연구 TradeLoader에 공통 심볼 제외(`--exclude-symbols`, `AUTO_REVAL_EXCLUDE_SYMBOLS`, `RESEARCH_EXCLUDE_SYMBOLS`) 경로 추가. 라이브 프로파일은 우선순위 설정을 즉시 반영해 종목/포지션/노출/레버리지/방향게이트를 보수화(`TOP_N_SYMBOLS=12`, `MAX_CONCURRENT_POSITIONS=4`, `MAX/LIVE/KELLY exposure=2.0`, `LEVERAGE_TARGET_MAX=8`, `LEVERAGE_REGIME_MAX_BEAR=5`, `LEVERAGE_REGIME_MAX_CHOP=3`, `REGIME_SIDE_BLOCK_LIST`에 `chop_long` 추가, `ALPHA_DIRECTION_GATE_MIN_CONF=0.605`, `ALPHA_DIRECTION_GATE_MIN_EDGE=0.065`) | `scripts/counterfactual_replay.py`, `scripts/analyze_entry_exit_reason_matrix.py`, `scripts/analyze_reval_batch_loss_drivers.py`, `scripts/auto_reval_db_reports.py`, `research/cf_engine.py`, `state/bybit.env`, `docs/CODE_MAP_v2.md` |
